# SilverSprinter — Luxury Sprinter Van Booking Site
...
Build a full-stack luxury sprinter van charter booking platform called **SilverSprinter**. The aesthetic is dark-gold luxury — think private jet charter meets high-end concierge. The platform serves three user types: consumers (individual bookings), corporate clients (team accounts with invoicing), and internal dispatchers (operations dashboard).
...
```bash
npx create-next-app@latest silversprinter --typescript --tailwind --app --src-dir
cd silversprinter
...
```env
# Database
DATABASE_URL="postgresql://user:password@localhost:5432/silversprinter"
...
AWS_S3_BUCKET="silversprinter-media"

# Email (Resend)
RESEND_API_KEY=""
FROM_EMAIL="hello@silversprinter.com"
...
| Event | Email | SMS |
|-------|-------|-----|
| `BOOKING_CONFIRMED` | Full confirmation with details + calendar invite | "Your SilverSprinter booking is confirmed for [date]. Booking ref: [ref]" |
...
*Generated by Claude · SilverSprinter build specification v1.0*
